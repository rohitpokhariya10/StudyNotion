...
const validatePublicEnv = (overrides = {}) =>
  spawnSync(process.execPath, ["scripts/validate-public-env.mjs"], {
    cwd: repositoryRoot,
    encoding: "utf8",
...
test("checked-in staging examples preserve the isolated test-mode contract", () => {
  const publicExample = readExampleEnv(".env.staging.example")
  ...
  const serverExample = readExampleEnv("server/.env.staging.example")
  ...
  const seedExample = readExampleEnv("server/.env.staging.seed.example")
