...
This workspace contains the Express API, its domain-oriented modular-monolith
boundaries, Mongoose models, provider adapters, operational scripts, and
backend tests. Run supported commands from the repository root unless a
section explicitly says otherwise.

## Local setup

1. Ensure MongoDB is running locally on `127.0.0.1:27017`.
2. Copy `apps/api/.env.example` to `apps/api/.env` if the latter is missing.
...
`npm --workspace studynotion-backend run seed` creates the local demo admin
listed above and marks the demo
...
`npm --workspace studynotion-backend run preflight:production` before release
or rollback. See
...
`../../docs/architecture/0002-catalog-v2-vertical-slice.md` for the
compatibility and cache decisions.
...
been migrated. See
`../../docs/architecture/0003-validated-v2-contract-foundation.md` for
validation, DTO-exposure, request-ID, and minor-unit money rules.
