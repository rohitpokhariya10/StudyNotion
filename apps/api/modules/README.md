...
`apps/api/modules` is the destination for incrementally migrated business
...
- Modules may depend on `apps/api/shared` infrastructure, but shared code must
...
The API remains a domain-oriented modular monolith. Existing mature slices
under `apps/api/domains` do not need to move here for directory symmetry. A new
module should expose cross-module behavior through its own `index.js`; consumers
must not import another module's private files. These rules are enforced by the
root `npm run architecture:check` command and documented by ADR 0011.
