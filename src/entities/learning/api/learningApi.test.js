...
import { store as applicationStore } from "../../../app/store"
...
describe("learning API application composition", () => {
  afterEach(() => {
    applicationStore.dispatch(learningApi.util.resetApiState())
    applicationStore.dispatch(setSession(false))
  })

  it("owns a dedicated reducer and clears enrolled learning data at logout", async () => {
    expect(rootReducer(undefined, { type: "test/init" })).toHaveProperty(
      learningApi.reducerPath
    )
    applicationStore.dispatch(setSession(true))
